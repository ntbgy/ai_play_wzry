...
def 手动训练数据截取(device_id, scrcpy_windows_name, 手工介入=True, flag_file_name='stop_flag.txt'):
    def 发送指令2(pyminitouch_device, 指令, sleep_time=0):
        try:
    ...
    try:
        while True:
            ...
            for i in range(6 * 1000):
                ...
                操作词典['图片号'] = str(i)
                手动移动指令 = handle_direction(W键按下, S键按下, A键按下, D键按下)
                ...
                if (手动移动指令 != ''
                    ...
                    自动 = 1
                    append_record(save_file_path, 操作词典)

                    新移动指令 = 操作词典['移动操作']
                    if 新移动指令 != 旧移动指令 and 新移动指令 != '无移动':
                        logger.info(f"移动操作，手动模式，{旧移动指令} -> {新移动指令}")
                        if not 发送指令2(pyminitouch_device, 新移动指令, 0.01):
                            break
                        # 连续移动，保持就好了，否则会鬼畜
                        旧移动指令 = 新移动指令
                    ...
                    if 新动作指令 not in ('无动作', '发起集合', '发起进攻', '发起撤退'):
                        ...
                        if not 发送指令2(pyminitouch_device, 新动作指令, 0.01):
                            break
                    ...
                    if 自动移动指令 != 旧移动指令 and 自动移动指令 != '无移动':
                        logger.debug(f"动作操作，自动模式：{自动动作指令}")
                        if not 发送指令2(pyminitouch_device, 自动移动指令, 0.01):
                            break
                    # 连续移动，保持就好了，否则会鬼畜
                    旧移动指令 = 自动移动指令
                    ...
                    if 自动动作指令 not in ('无动作', '发起集合', '发起进攻', '发起撤退'):
                        logger.debug(f"动作操作，自动模式：{自动动作指令}")
                        if not 发送指令2(pyminitouch_device, 自动动作指令, 0.01):
                            break
...
def singele_run_game(dir_path, device_id, scrcpy_windows_name, flag_file_name):
    """
    运行多局游戏
    """
    for i in range(1, 2):
