...
import yao_skill_logic
from auto_game import se, check_stop, back_to_room, start_next_game
...
def main():
    ...
    yao_thread = threading.Thread(target=yao_skill_logic.run, args=(yao_skill_queue,), daemon=True)
    thread_check_stop = threading.Thread(target=check_stop)

    thread1.daemon = True
    thread2.daemon = True
    yao_thread.daemon = True
    thread1.start()
    thread2.start()
    yao_thread.start()
    thread_check_stop.start()
